...
module.exports = Router()
  ...
  .post('/create', authenticate, async (req, res, next) => {
    try {
      ...
      const yourProfile = await profile.createYourProfile(req.user.uuid);
  ...
  .get('/:id', authenticate, async (req, res, next) => {
    try {
      const profile = await Profile.getProfileById(req.user.uuid);
      const yourProfile = await profile.getYourProfile(req.user.uuid);
      res.send(yourProfile);
    } catch (error) {
      next(error);
    }
  ...
  .patch('/:id', authenticate, async (req, res, next) => {
    try {
      const profile = await Profile.updateProfile(req.user.uuid, req.body);
      res.send(profile);
    } catch (error) {
      next(error);
    }
